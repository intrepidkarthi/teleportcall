...
var Event, Kimd, Loader, Main, Screens, Scroll, UI_Slider, Utils, VideoPlayer, VideoPlayer_Controls, VideoPlayer_HTML, VideoPlayer_Templates, W, main,
  __bind = function(fn, me){ return function(){ return fn.apply(me, arguments); }; };

Loader = (function() {
  function Loader(options) {
    var imgInc, imgLength, pics;
    this.container = options.container, this.elm = options.elm, this.stepback = options.stepback, this.complete = options.complete;
    if (this.elm) {
...
Scroll = (function() {
  function Scroll(options) {
    ...
    this.scrollbar = {
      ...
      thumbHeight: this.scrollbarThumb.height()
    };
  ...
  Scroll.prototype._initEvents = function() {
    return $('input').on(Event.CLICK, function() {
      $('input').blur();
      return $(this).focus();
    });
  };
  ...
  Scroll.prototype.mouseDown = function(e) {
    ...
    if (klass.match('scrollbar')) {
      ...
      this.initScrollY = W.scrollTop.perc * (this.scrollbar.trackHeight - this.scrollbar.thumbHeight);
      this.initY = e.pageY - this.initScrollY;
    } else {
      this.isScrollbar = false;
      this.ease = 1;
      this.initScrollY = W.scrollTop.perc * W.scrollTop.limit;
      this.initY = e.pageY;
    }
  ...
  Scroll.prototype.mouseMove = function(e) {
    if (this.isMouseDown) {
      ...
      if (this.isScrollbar === true) {
        ...
        return W.scrollTop.perc = this.moveY / (this.scrollbar.trackHeight - this.scrollbar.thumbHeight);
      } else {
        this.prevY = this.moveY;
        this.moveY = -this.initScrollY + e.pageY - this.initY;
        return W.scrollTop.perc = -this.moveY / W.scrollTop.limit;
      }
    }
  };
  ...
  Scroll.prototype.mouseUp = function() {
    if (this.isMouseMoving === true && this.isScrollbar === false) {
      W.scrollTop.perc = -(this.moveY + (this.moveY - this.prevY) * 10) / W.scrollTop.limit;
    }
    this.ease = 0.1;
    return this.isScrollbar = this.isMouseDown = this.isMouseMoving = false;
  };

  Scroll.prototype.mouseWheel = function(deltaY) {
    var initScrollY, moveY;
    initScrollY = W.scrollTop.perc * W.scrollTop.limit;
    moveY = -initScrollY - deltaY;
    return W.scrollTop.perc = -moveY / W.scrollTop.limit;
  };
  ...
  Scroll.prototype.goTo = function(pos) {
    return W.scrollTop.perc = pos / W.scrollTop.limit;
  };

  Scroll.prototype.resize = function() {
    this.overviewHeight = this.overview.height();
    this.viewportHeight = this.viewport.height();
    W.scrollTop.limit = this.overview.height() - this.viewportHeight;
    this.scrollbarContainer.height(W.wh);
    return this.scrollbar.trackHeight = this.scrollbarTrack.height();
  };

  Scroll.prototype.update = function() {
    if (W.scrollTop.perc < 0) {
      W.scrollTop.perc = 0;
    } else if (W.scrollTop.perc > 1) {
      W.scrollTop.perc = 1;
    }
    W.scrollTop.real = W.scrollTop.perc * W.scrollTop.limit;
    W.scrollTop.calc += (W.scrollTop.real - W.scrollTop.calc) * this.ease;
    Normalize.transform(this.overview[0], 'translate3d(0, ' + (-W.scrollTop.calc) + 'px, 0)');
    return Normalize.transform(this.scrollbarThumb[0], 'translate3d(0, ' + (W.scrollTop.perc * (this.scrollbar.trackHeight - this.scrollbar.thumbHeight)) + 'px, 0)');
  };
...
VideoPlayer_Templates = (function() {
  ...
  VideoPlayer_Templates.PLAY_BUTTON = "<span class=\"play-button\"></span>";

  VideoPlayer_Templates.PAUSE_BUTTON = "<span class=\"pause-button\"></span>";
...
W = (function() {
  ...
  W.isTablet = $('body').hasClass('tablet') ? true : false;
...
Kimd = (function() {
  function Kimd() {
    ...
    this._closePopin = __bind(this._closePopin, this);
    this._openPopin = __bind(this._openPopin, this);
    this._onMouseWheel = __bind(this._onMouseWheel, this);
    this._onMouseUp = __bind(this._onMouseUp, this);
    this._onMouseMove = __bind(this._onMouseMove, this);
    this._onMouseDown = __bind(this._onMouseDown, this);
    this._initEvents = __bind(this._initEvents, this);
    this.globalScroll = new Scroll();
    ...
    this.popinsButtons = $('.popin-button');
    this.closePopinsButtons = $('.popin .close-button');
    this.backgrounds = $('.background');
    this.gradient = $('.gradient');
    this.screens = new Screens();
    this._elementTouched = null;
    this._initEvents();
  }

  Kimd.prototype._initEvents = function() {
    W.body.on(Event.MOUSEDOWN, this._onMouseDown).on(Event.MOUSEMOVE, this._onMouseMove).on(Event.MOUSEUP, this._onMouseUp).on(Event.WHEEL, this._onMouseWheel);
    this.popinsButtons.on(Event.CLICK, this._openPopin);
    return this.closePopinsButtons.on(Event.CLICK, this._closePopin);
  };

  Kimd.prototype._onMouseDown = function(event) {
    var e, _ref;
    e = event.type === 'touchstart' ? event.originalEvent.touches[0] : event;
    this._elementTouched = $(event.target);
    if ((_ref = event.which) === 0 || _ref === 1) {
      this.globalScroll.mouseDown(e);
    }
    return event.preventDefault();
  };

  Kimd.prototype._onMouseMove = function(event) {
    var e, _ref;
    e = event.type === 'touchmove' ? event.originalEvent.touches[0] : event;
    if ((_ref = event.which) === 0 || _ref === 1) {
      this.globalScroll.mouseMove(e);
    }
    return event.preventDefault();
  };

  Kimd.prototype._onMouseUp = function(event) {
    var e, element, _ref;
    e = event.type === 'touchend' ? event.originalEvent.touches[0] : event;
    if (this._elementTouched.is('a')) {
      element = this._elementTouched;
    } else {
      element = this._elementTouched.parents('a');
    }
    if (W.isTablet && element.attr('href')) {
      window.open(element.attr('href'), '_blank');
    }
    if ((_ref = event.which) === 0 || _ref === 1) {
      this.globalScroll.mouseUp();
    }
    return event.preventDefault();
  };

  Kimd.prototype._onMouseWheel = function(e) {
    var deltaY;
    deltaY = e.originalEvent.deltaY || -e.originalEvent.wheelDeltaY || e.deltaY;
    return this.globalScroll.mouseWheel(deltaY);
  };
  ...
  Kimd.prototype._openPopin = function(e) {
    ...
    return setTimeout((function(_this) {
      return function() {
        return popin.css('opacity', 1);
      };
    })(this), 10);
  };

  Kimd.prototype._closePopin = function(e) {
    var $this, popin;
    $this = $(e.currentTarget);
    popin = $this.parents('.popin');
    popin.css('opacity', 0);
    return setTimeout((function(_this) {
      return function() {
        return popin.css('display', 'none');
      };
    })(this), 1000);
  };

  Kimd.prototype.resize = function() {
    var dims;
    W.body.css({
      'height': W.wh,
      'width': W.ww
    });
    this.globalScroll.resize();
    dims = Utils.getCoverSizeImage(1280, 720, W.ww, W.wh);
    this.backgrounds.css({
      'width': dims.width,
      'height': dims.height,
      'margin-top': dims.top,
      'margin-left': dims.left
    });
    return this.screens.resize();
  };

  Kimd.prototype.update = function() {
    this.globalScroll.update();
    return this.screens.update();
  };
...
Screens = (function() {
  function Screens() {
    this.update = __bind(this.update, this);
    this.resize = __bind(this.resize, this);
    this.container = $('#screens');
    this.parts = [];
    this.$parts = this.container.find('.part');
    this.$parts.each((function(_this) {
      return function(key, part) {
        var item;
        item = {};
        item.elm = $(part);
        item.mask = item.elm.children('.mask');
        item.iphone = item.elm.find('.iphone-container');
        item.screen = item.elm.find('.iphone-screen');
        return _this.parts.push(item);
      };
    })(this));
  }

  Screens.prototype.resize = function() {
    this.container.height(this.parts.length * W.wh);
    this.$parts.height(W.wh);
    return this.pHeight = this.container.height();
  };

  Screens.prototype.update = function() {
    var duration, index, realPos, startPos;
    startPos = 0;
    duration = this.pHeight + W.wh;
    if (W.scrollTop.calc > startPos && W.scrollTop.calc < startPos + duration) {
      realPos = W.scrollTop.calc;
      index = ~~realPos / W.wh;
      return $.each(this.parts, (function(_this) {
        return function(key, part) {
          var pcDuration, pcStartPos, pos, rotate;
          pcStartPos = key * W.wh - W.wh;
          pcDuration = 2 * W.wh;
          rotate = 0;
          if (realPos > pcStartPos && realPos < pcDuration + pcStartPos) {
            pos = realPos - W.wh - pcStartPos;
            if (key === 7) {
              if (pos < 0) {
                rotate = 0;
              } else if (pos > W.wh) {
                ...
                rotate = 180 + (1 + (realPos - pcStartPos) / W.wh) * 90;
              }
              Normalize.transform(part.iphone[0], 'rotate(' + rotate + 'deg)');
            }
            if (key === 8) {
              if (pos < 0) {
                rotate = 180 + (1 + (realPos - pcStartPos) / W.wh) * 90;
            ...
            return Normalize.transform(part.mask[0], 'translate3d(0, ' + pos + 'px, 0)');
          }
        };
      })(this));
    }
...
Main = (function() {
  ...
  Main.prototype._onResize = function() {
    ...
    if (this.kimd) {
      return this.kimd.resize();
    }
  };

  Main.prototype.update = function() {
    if (this.kimd) {
      return this.kimd.update();
    }
  };
